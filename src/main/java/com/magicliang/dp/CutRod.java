...
/**
 * 切割钢条问题
 * 动态规划仔细安排求解顺序，对每个子问题只求解一次，并将结果保存下来，也就是 CLRS 中所说的表方法。
 * 这是典型的 time-memory trade-off 的例子。可以把一个指数(exponential)时间的解，转化为一个多项式（polynomial）时间的解。
 */
public class CutRod {
    ...
    /**
     * 自底向上的备忘录方法。
     * 此方法一般要恰当定义子问题的规模的概念。使得任何子问题的求解都只依赖于“更小的”子问题的求解。
     * 因而我们可以将子问题按照规模排序，按由小至大的顺序进行求解。当求解某个子问题时，它所依赖的那些
     * 更小的子问题都已求解完毕。
     *
     * @param price 每种切割方式的价格数组
     * @param n     要切割的钢条的总长度
